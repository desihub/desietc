...
etcreplay --traceback --verbose --gfa-calib ~/Data/DESI/ETC/GFA_calib.fits --sky-calib  ~/Data/DESI/ETC/SKY_calib.fits --inpath ~/Data/DESI --outpath ~/Data/DESI/ETC --night 20201218 --expid 68630 --overwrite --parallel
...
def etcoffline(args):
    ...
    try:
        ...
        if args.expid is not None:
            ...
            for token in args.expid.split(","):
                ...
                if len(limits) == 1:
                    ...
                    print(
                        'Invalid --expid (should be N or N1-N2): "{0}"'.format(
                            args.expid
                        )
                    )
                    sys.exit(-1)
        ...
        get_exposures = lambda: set(
            [
                int(p.name)
                for p in nightpath.glob("????????")
                if expid_pattern.match(p.name)
            ]
        )
...
def main():
    ...
    parser.add_argument(
        "--debug", action="store_true", help="provide verbose and debugging output"
    )
    parser.add_argument(
        "--traceback",
        action="store_true",
    ...
    parser.add_argument(
        "--batch", action="store_true", help="Process all existing exposures on night"
    )
    parser.add_argument(
        "--watch",
        action="store_true",
    ...
    parser.add_argument(
        "--parallel", action="store_true", help="Process GFA cameras in parallel"
    )
